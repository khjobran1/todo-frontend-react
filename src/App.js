import React from 'react';
import { BrowserRouter as Router, Route} from 'react-router-dom';
import Header from './components/layout/Header'
import Todos from './components/Todos'
import './App.css';
import AddTodo from './components/AddTodo';
import About from './components/pages/About';
// import uuid from 'uuid';
import axios from 'axios';

class App extends React.Component {
  state = {
    todos: []
  }

  componentDidMount(){
    axios.get('http://localhost:3005/todos')
    .then(res => this.setState({ todos: res.data.data }))
  }
  // Toggle Complete
  markComplete = (id) => {
    var flag = false;
    this.setState({ todos: this.state.todos.map(todo => {
      if (todo.id === id){
        flag = !todo.completed
      }
      return todo;
    })});

    axios({ method: 'POST', 
    url: 'http://localhost:3005/complete', 
    headers: {'Content-Type':'application/x-www-form-urlencoded'}, 
    params: { id, completed: flag} })
      .then(res => this.setState({ todos: this.state.todos.map(todo => {
        if (todo.id === id){
          todo.completed = !todo.completed
        }
        return todo;
      })}));
      
    
  }

  // Delete Todo
  telTodo = (id) => {
    axios({ method: 'POST', 
    url: 'http://localhost:3005/destroy', 
    headers: {'Content-Type':'application/x-www-form-urlencoded'}, 
    params: { id } })
      .then(res => this.setState({ todos: [...this.state.todos.filter(todo => todo.id !== id)] }));
  }

  // Add Todo
  addTodo = (title) => {
    axios({ method: 'POST', 
    url: 'http://localhost:3005/addtodo', 
    headers: {'Content-Type':'application/x-www-form-urlencoded'}, 
    params: { title: title, completed: false } })
      .then(res => this.setState({ todos: [...this.state.todos, res.data.data] }))
  }
  render() {
    return (
      <Router>
        <div className="App">
        <div className="container">
            <Header />
            <Route exact path='/' render={props => (
              <React.Fragment>
                <AddTodo addTodo={this.addTodo}/>
                 <Todos todos={this.state.todos} markComplete={this.markComplete}
                   delTodo={this.telTodo} />
              </React.Fragment>
            )}/>
            <Route path='/about' component={About} />
        </div>
      </div>
      </Router>
    );
  }
}

export default App;
